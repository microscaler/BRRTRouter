...
fn main() -> io::Result<()> {
    ...
    // Log comprehensive startup context
    use tracing::info;
    
    info!(
        spec_path = %spec_path.display(),
        static_dir = ?args.static_dir,
        doc_dir = %args.doc_dir.display(),
        stack_size = config.stack_size,
        routes_count = routes.len(),
        hot_reload = args.hot_reload,
        has_test_api_key = args.test_api_key.is_some(),
    ...
    if let Some(sd) = &args.static_dir { println!("[startup] static_dir={}", sd.display()); }
    ...
    info!(
        ...
        max_requests = max,
        has_security_config = app_config.security.is_some(),
        has_http_config = app_config.http.is_some(),
                ...
                match scheme {
                    SecurityScheme::ApiKey { name, location, .. } => {
                        match location.as_str() {
                            "header" => {
                                let target = self.header_override.as_ref().map(|s| s.as_str()).unwrap_or_else(|| name);
                                req.headers.get(&target.to_ascii_lowercase()).map(|s| s.as_str()) == Some(self.key.as_str())
                            }
            ...
            match scheme {
                ...
                SecurityScheme::Http { ref scheme, .. } if scheme.eq_ignore_ascii_case("bearer") => {
                    ...
                    if let Some(pa) = sec_cfg.and_then(|s| s.propelauth.as_ref()) {
                        ...
                        let issuer_opt: Option<&str> = pa.issuer.as_deref().or_else(|| Some(pa.auth_url.as_str()));
