...
fn test_validator_cache_basic_functionality() {
    ...
    assert_eq!(cache.size(), 1, "Cache should contain 1 validator after first compilation");
    ...
    assert_eq!(cache.size(), 1, "Cache size should remain 1 (reused existing)");
...
fn test_validator_cache_with_app_service() {
    ...
    assert_eq!(service.validator_cache.size(), 0, 
...
fn test_validator_validates_correctly() {
    ...
    let validator = cache.get_or_compile("test_handler", "request", None, &schema)
    ...
    assert!(!validator.is_valid(&invalid_data1), 
    ...
    assert!(!validator.is_valid(&invalid_data2), 
    ...
    assert!(!validator.is_valid(&invalid_data3), 
...
fn test_cache_thread_safety() {
    ...
    for i in 0..10 {
        ...
        let handle = thread::spawn(move || {
            ...
            let validator = cache_clone.get_or_compile(
                "concurrent_handler",
                "request",
                None,
                &schema_clone
    ...
    // Collect results
    let validators: Vec<_> = handles.into_iter()
        .map(|h| h.join().unwrap())
        .collect();
    ...
    for (i, validator) in validators.iter().enumerate().skip(1) {
        assert!(Arc::ptr_eq(first_ptr, validator), 
                "Thread {} got a different validator Arc", i);
    }
...
fn test_invalid_schema_not_cached() {
    ...
    assert!(result2.is_none(), "Invalid schema should return None on second attempt");
