...
fn test_metrics_middleware_auto_preregisters_paths() {
    ...
    let dispatcher = Arc::new(RwLock::new(
    ...
    assert!(stats.contains_key("/users/{id}"), "Should contain /users/{{id}}");
    assert!(stats.contains_key("/posts/{id}"), "Should contain /posts/{{id}}");
...
fn test_metrics_middleware_preregistration_with_base_path() {
    ...
    assert!(stats.contains_key("/api/v1/users"), "Should contain full path with base");
    assert!(stats.contains_key("/api/v1/posts"), "Should contain full path with base");
}
...
fn test_metrics_middleware_preregisters_parameterized_paths() {
    ...
    let stats = metrics.path_stats();
    assert_eq!(stats.len(), 2);
    assert!(stats.contains_key("/users/{id}"), "Should contain parameterized path");
    assert!(stats.contains_key("/users/{id}/posts/{post_id}"), "Should contain nested parameterized path");
