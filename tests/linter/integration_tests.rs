...
fn test_bad_operation_id_casing() {
    ...
    assert!(
        locations.iter().any(|l| l.contains("/users/{id} POST")),
...
fn test_missing_operation_id() {
    ...
    assert!(
        locations.iter().any(|l| l.contains("/pets/{id} DELETE")),
...
fn test_invalid_schema_format() {
    ...
    // Should find required field format warning
    let required_format_warnings: Vec<_> = issues
        .iter()
        .filter(|i| i.kind == "required_field_format" && i.severity == LintSeverity::Warning)
        .collect();

    assert!(
        !required_format_warnings.is_empty(),
        "Should find required field format warning. Found: {:?}",
        issues
    );
...
fn test_mixed_issues() {
    ...
    // - Multiple missing schema ref errors
    assert!(
        errors.len() >= 4,
    ...
    // Verify missing schema ref errors
    let missing_ref_errors: Vec<_> = errors
        .iter()
        .filter(|i| i.kind == "missing_schema_ref")
        .collect();
    assert!(
        missing_ref_errors.len() >= 3,
        "Should find at least 3 missing schema ref errors"
    );

    // Verify warnings
    assert!(
        warnings.len() >= 2,
        "Should find at least 2 warnings"
    );
}
