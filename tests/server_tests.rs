...
fn test_panic_recovery() {
    ...
    let server = CustomServerTestFixture::with_handler(
        "panic",
        panic_handler,
        "/panic",
        Method::GET,
    ...
    let resp = send_request(&server.addr(), "GET /panic HTTP/1.1\r\nHost: localhost\r\n\r\n");
    let (status, body) = parse_response(&resp);
...
fn test_status_201_json() {
    ...
    let resp = send_request(&server.addr(), "POST /created HTTP/1.1\r\nHost: localhost\r\n\r\n");
    let (status, body) = parse_response(&resp);
...
fn test_text_plain_error() {
    ...
    let resp = send_request(&server.addr(), "GET /text HTTP/1.1\r\nHost: localhost\r\n\r\n");
    let (status, body) = parse_response(&resp);
...
fn test_response_body_validation_failure() {
    ...
    let resp = send_request(&server.addr(), "GET /bad HTTP/1.1\r\nHost: localhost\r\n\r\n");
    let (status, _body) = parse_response(&resp);
    assert_eq!(status, 400);
