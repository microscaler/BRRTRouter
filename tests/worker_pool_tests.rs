...
fn test_worker_pool_shed_mode() {
    ...
    unsafe {
        dispatcher.register_handler_with_pool_config("slow_handler", move |req: HandlerRequest| {
...
fn test_worker_pool_block_mode() {
    ...
    unsafe {
        dispatcher.register_handler_with_pool_config("fast_handler", move |req: HandlerRequest| {
    ...
    // Get the worker pool to test dispatch directly
    let pool = dispatcher.worker_pools.get("fast_handler").expect("Pool not found").clone();
    ...
    for _i in 0..20 {
        ...
        match pool.dispatch(req) {
            ...
            Err(response) => {
                ...
                assert_eq!(response.status, 429, "Expected 429 status for timed out request");
                _timeout_count += 1;
            }
        }
    ...
    assert!(success_count > 15, "Expected most requests to succeed in block mode, but only {} succeeded", success_count);
}
...
fn test_worker_pool_metrics() {
    ...
    let pool = dispatcher.worker_pools.get("metrics_handler").expect("Pool not found").clone();
    ...
    let (queue_depth, _shed_count, dispatched, _completed) = metrics_after.get("metrics_handler").unwrap();
    
    // Check that we dispatched some requests
    assert!(*dispatched >= 5, "Expected at least 5 dispatched, got {}", dispatched);
    
    // Queue should be draining or empty
    assert!(*queue_depth <= 5, "Queue depth should be reasonable, got {}", queue_depth);
}
