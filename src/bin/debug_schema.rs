...
fn main() {
    ...
    println!("📋 Input schema: {}", serde_json::to_string_pretty(&user_schema).unwrap());
    
    // Call our function
    let result = brrtrouter::generator::schema::build_complete_example_object(&user_schema);
    
    println!("✅ Generated object: {}", serde_json::to_string_pretty(&result).unwrap());
    ...
    if let Some(email) = result.get("email") {
        ...
        println!("Present fields: {:?}", result.as_object().map(|o| o.keys().collect::<Vec<_>>()));
    }
} 