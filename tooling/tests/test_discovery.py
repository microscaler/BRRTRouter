"""Tests for brrtrouter_tooling.discovery (suites)."""

from pathlib import Path


class TestSuites:
    def test_suites_with_bff_empty_when_no_bff_config(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.discovery import suites_with_bff

        (tmp_path / "openapi" / "x").mkdir(parents=True)
        assert suites_with_bff(tmp_path) == []

    def test_suites_with_bff_returns_suites_with_config(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.discovery import suites_with_bff
    ...
    def test_iter_bffs_yields_bff_name_and_suite(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.discovery import iter_bffs
    ...
    def test_bff_suite_config_path(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.discovery import bff_suite_config_path

        p = bff_suite_config_path(tmp_path, "acct")
        assert p == tmp_path / "openapi" / "acct" / "bff-suite-config.yaml"

    def test_service_to_suite_finds_spec(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.discovery import service_to_suite
    ...
    def test_service_to_suite_none_when_missing(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.discovery import service_to_suite

        assert service_to_suite(tmp_path, "nonexistent") is None

    def test_suite_sub_service_names_empty_when_no_suite_dir(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.discovery import suite_sub_service_names

        assert suite_sub_service_names(tmp_path, "acct") == []

    def test_suite_sub_service_names_returns_dirs_with_openapi_yaml(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.discovery import suite_sub_service_names
    ...
    def test_tilt_service_names_includes_bff_and_services(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.discovery import tilt_service_names
    ...
    def test_tilt_service_names_empty_when_no_bff_config(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.discovery import tilt_service_names
