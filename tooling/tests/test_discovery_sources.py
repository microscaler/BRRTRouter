"""Tests for brrtrouter_tooling.discovery.sources: helm, kind, tiltfile, bff-suite, openapi localhost."""
...
def test_discover_helm_missing_dir(tmp_path: Path) -> None:
    got = discover_helm(tmp_path)
    assert got == {}


def test_discover_helm_one_values(tmp_path: Path) -> None:
...
def test_discover_helm_name_from_stem(tmp_path: Path) -> None:
    d = tmp_path / "helm" / "rerp-microservice" / "values"
    d.mkdir(parents=True)
...
def test_discover_kind_host_ports_missing(tmp_path: Path) -> None:
    got = discover_kind_host_ports(tmp_path)
    assert got == []


def test_discover_kind_host_ports(tmp_path: Path) -> None:
...
def test_discover_tiltfile_missing(tmp_path: Path) -> None:
    got = discover_tiltfile(tmp_path)
    assert got == {}


def test_discover_tiltfile_ports_block(tmp_path: Path) -> None:
...
def test_discover_bff_suite_config_empty(tmp_path: Path) -> None:
...
def test_discover_openapi_bff_localhost_empty(tmp_path: Path) -> None:
...
def test_discover_openapi_suite_microservice_localhost_empty(tmp_path: Path) -> None:
