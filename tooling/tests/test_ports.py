"""Tests for brrtrouter_tooling.ports (PortRegistry, validate, reconcile, fix_duplicates)."""

from pathlib import Path


class TestPortRegistry:
    def test_load_empty_creates_default(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.ports import PortRegistry
    ...
    def test_assign_and_get(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.ports import PortRegistry
    ...
    def test_assign_idempotent_without_force(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.ports import PortRegistry
    ...
    def test_release_port(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.ports import PortRegistry
...
class TestValidate:
    def test_validate_no_sources_ok(self, tmp_path: Path, capsys) -> None:
        ...
        assert rc == 0
        assert "No port conflicts" in out or "No hard conflicts" in out

    def test_validate_json_out(self, tmp_path: Path, capsys) -> None:
        from brrtrouter_tooling.ports import PortRegistry, validate
...
class TestReconcile:
    def test_reconcile_adds_helm_only_service(self, tmp_path: Path, capsys) -> None:
        from brrtrouter_tooling.ports import PortRegistry, reconcile
...
class TestFixDuplicates:
    def test_fix_duplicates_none(self, tmp_path: Path, capsys) -> None:
        from brrtrouter_tooling.ports import PortRegistry, fix_duplicates
    ...
    def test_fix_duplicates_dry_run(self, tmp_path: Path, capsys) -> None:
        from brrtrouter_tooling.ports import PortRegistry, fix_duplicates
