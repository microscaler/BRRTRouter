"""TDD: tests for brrtrouter_tooling.workspace.bff.generate_system (hauliage bff generate-system)."""
...
class TestUpdateRefsInValue:
    ...
    def test_exact_match_rewritten(self) -> None:
        from brrtrouter_tooling.workspace.bff.generate_system import _update_refs_in_value
...
class TestDiscoverSubServices:
    def test_system_dir_missing_returns_empty(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.workspace.bff.generate_system import discover_sub_services
    ...
    def test_one_subdir_with_openapi_returns_service(self, tmp_path: Path) -> None:
        ...
        assert got["svc"]["spec"] == tmp_path / "svc" / "openapi.yaml"
        assert got["svc"]["base_path"] == "/api/v1/svc"
    ...
    def test_two_subdirs_returns_both(self, tmp_path: Path) -> None:
        ...
        for s in ["a", "b"]:
            ...
            (tmp_path / s / "openapi.yaml").write_text("openapi: 3.1.0\ninfo: {}\npaths: {}")
        got = discover_sub_services(tmp_path, "s")
        assert set(got.keys()) == {"a", "b"}
        assert got["a"]["base_path"] == "/api/v1/a"
        assert got["b"]["base_path"] == "/api/v1/b"

    def test_skips_hidden_dirs(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.workspace.bff.generate_system import discover_sub_services

        (tmp_path / ".hidden").mkdir(parents=True)
        (tmp_path / ".hidden" / "openapi.yaml").write_text("openapi: 3.1.0\ninfo: {}\npaths: {}")
        (tmp_path / "ok").mkdir()
        (tmp_path / "ok" / "openapi.yaml").write_text("openapi: 3.1.0\ninfo: {}\npaths: {}")
        got = discover_sub_services(tmp_path, "s")
    ...
    def test_skips_dirs_without_openapi_yaml(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "no-spec").mkdir(parents=True)
        (tmp_path / "with-spec").mkdir()
        (tmp_path / "with-spec" / "openapi.yaml").write_text("openapi: 3.1.0\ninfo: {}\npaths: {}")
        got = discover_sub_services(tmp_path, "s")
    ...
    def test_base_path_uses_system_and_service(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "identity" / "openapi.yaml").write_text("openapi: 3.1.0\ninfo: {}\npaths: {}")
        got = discover_sub_services(tmp_path, "hauliage")
        assert got["identity"]["base_path"] == "/api/v1/identity"
...
class TestListSystemsWithSubServices:
    ...
    def xtest_system_with_sub_services_included(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.workspace.bff.generate_system import list_systems_with_sub_services
    ...
    def xtest_system_without_sub_services_excluded(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.workspace.bff.generate_system import list_systems_with_sub_services
    ...
    def xtest_multiple_systems_sorted(self, tmp_path: Path) -> None:
        from brrtrouter_tooling.workspace.bff.generate_system import list_systems_with_sub_services
...
class TestGenerateSystemBffSpec:
    ...
    def test_output_path_default_when_none(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "a").mkdir(parents=True)
        (tmp_path / "a" / "openapi.yaml").write_text(_minimal_spec())
        generate_system_bff_spec(tmp_path, "x", output_path=None)
        default = tmp_path / "openapi_bff.yaml"
    ...
    def test_idempotent_second_run_same_content(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "v" / "openapi.yaml").write_text(_minimal_spec())
        out = tmp_path / "openapi_bff.yaml"
        generate_system_bff_spec(tmp_path, "s", output_path=out)
        c1 = out.read_text()
        generate_system_bff_spec(tmp_path, "s", output_path=out)
    ...
    def test_schemas_prefixed_with_service_pascal(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "my-svc").mkdir(parents=True)
        (tmp_path / "my-svc" / "openapi.yaml").write_text(
            _minimal_spec(
