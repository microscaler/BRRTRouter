"""Tests for brrtrouter_tooling.workspace.ci.get_latest_tag (hauliage ci get-latest-tag)."""
...
from brrtrouter_tooling.workspace.ci.get_latest_tag import (
...
class TestGetLatestTag:
    def test_returns_latest_tag_from_github_api(self) -> None:
        with patch(
            "brrtrouter_tooling.workspace.ci.get_latest_tag.urlopen",
    ...
    def test_retries_on_http_error_then_succeeds(self) -> None:
        ...
        with (
            patch("brrtrouter_tooling.workspace.ci.get_latest_tag.urlopen") as mock_urlopen,
            patch("brrtrouter_tooling.workspace.ci.get_latest_tag.time.sleep") as mock_sleep,
    ...
    def test_exhausts_retries_and_raises(self) -> None:
        ...
        http_error = HTTPError("url", 503, "Service Unavailable", {}, None)
        with (
            patch("brrtrouter_tooling.workspace.ci.get_latest_tag.urlopen", side_effect=http_error),
            patch("brrtrouter_tooling.workspace.ci.get_latest_tag.time.sleep"),
            patch("sys.stderr", new=StringIO()) as fake_err,
        ):
