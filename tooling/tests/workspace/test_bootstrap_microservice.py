"""Tests for brrtrouter_tooling.workspace.bootstrap.microservice."""
...
class TestToSnakeCase:
    def test_hyphens(self):
        assert to_snake_case("identity") == "identity"
...
class TestToPascalCase:
    def test_hyphens(self):
        assert to_pascal_case("identity") == "Identity"
...
class TestUpdateWorkspaceCargoToml:
    ...
    def test_preserves_content_after_members_array(self, tmp_path: Path):
        ...
        assert '"bar/gen"' in text
        assert '"bar/impl"' in text
    ...
    def test_no_op_if_already_member(self, tmp_path: Path):
        cargo = tmp_path / "Cargo.toml"
        cargo.write_text(
            '[workspace]\nmembers = ["hauliage/baz/gen", "hauliage/baz/impl"]\nresolver = "2"\n'
        )
        update_workspace_cargo_toml("baz", cargo)
        assert cargo.read_text().count('"hauliage/baz/gen"') == 1
        assert cargo.read_text().count('"hauliage/baz/impl"') == 1
...
class TestUpdateTiltfile:
    ...
    def test_deps_preserves_resource_deps(self, tmp_path: Path) -> None:
        ...
        tilt.write_text(
            ...
            "    deps=['./microservices/a/gen/Cargo.toml', './microservices/a/impl/Cargo.toml']\n"
        ...
        assert "'./microservices/b/gen/Cargo.toml'" in text
        assert "'./microservices/b/impl/Cargo.toml'" in text
...
class TestEnsureImplScaffold:
    ...
    def test_creates_impl_when_gen_exists_and_impl_missing(self, tmp_path: Path) -> None:
        ...
        assert "hauliage_company" in cargo
        assert "hauliage_company_gen" in cargo
...
class TestRegenerateImplStubsCreatesImplWhenMissing:
    ...
    def test_creates_impl_and_calls_stub_generator(self, tmp_path: Path) -> None:
        ...
        with patch(
            "brrtrouter_tooling.workspace.bootstrap.microservice.generate_impl_stubs_with_brrtrouter"
        ) as m_stubs:
