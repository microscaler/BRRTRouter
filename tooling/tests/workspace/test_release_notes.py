"""Tests for brrtrouter_tooling.workspace.release.notes."""
...
from brrtrouter_tooling.workspace.release.notes import (
    DEFAULT_TEMPLATE,
    _call_anthropic,
    _call_openai,
    _get_commits_since,
...
class TestRun:
    def test_fails_without_previous_tag(self, tmp_path: Path) -> None:
        with patch(
            "brrtrouter_tooling.workspace.release.notes._get_previous_tag", return_value=None
        ):
    ...
    def test_succeeds_with_since_tag_and_mocked_openai(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch(
                "brrtrouter_tooling.workspace.release.notes._call_openai",
    ...
    def test_succeeds_stdout_when_no_output_path(
        ...
        with (
            patch(
                "brrtrouter_tooling.workspace.release.notes._get_previous_tag",
                return_value="v0.9.0",
            ),
            patch(
                "brrtrouter_tooling.workspace.release.notes._get_commits_since", return_value=["a"]
            ),
            patch(
                "brrtrouter_tooling.workspace.release.notes._call_openai", return_value="Notes here"
            ),
        ):
    ...
    def test_run_provider_anthropic_uses_call_anthropic(
        ...
        with (
            ...
            patch(
                "brrtrouter_tooling.workspace.release.notes._call_anthropic",
                return_value="# Release v1.0.0\n\nClaude notes.",
            ),
        ):
            rc = run(
    ...
    def test_run_returns_1_when_empty_body(self, tmp_path: Path) -> None:
        with (
            ...
            patch("brrtrouter_tooling.workspace.release.notes._call_openai", return_value=""),
        ):
            rc = run(
                tmp_path,
                "1.0.0",
    ...
    def test_run_output_path_creates_parent_dirs(self, tmp_path: Path) -> None:
        ...
        with (
            patch(
                "brrtrouter_tooling.workspace.release.notes._get_commits_since", return_value=["x"]
            ),
            patch(
                "brrtrouter_tooling.workspace.release.notes._call_openai",
                return_value="# Release v1.0.0\n\nDone.",
