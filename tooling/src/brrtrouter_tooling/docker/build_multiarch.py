...
def run(  # noqa: C901
    ...
    base_image_local: str | None = None,
) -> int:
    """Build binaries (via build_cmd), copy, buildx images, manifest, optional push. Returns 0 or 1.

    base_image_local: If set (e.g. 'rerp/base'), use FROM base_image_local:{arch} in per-arch
    Dockerfiles and tag locally built base as base_image_local:{arch}. If None, use ARG BASE_IMAGE
    and ghcr.io/owner/base_image_name for pull/build.
    """
    ...
    template_path = root / docker_dir / "microservices" / "Dockerfile.template"
    ...
    for arch in ["amd64", "arm64", "arm7"]:
        ...
        base_img_local_tag = (
            f"{base_image_local}:{arch}" if base_image_local else f"{base_image_name}:{arch}"
        )
        ...
        check_local = subprocess.run(
            ["docker", "images", "-q", base_img_local_tag],
            capture_output=True,
            text=True,
            cwd=str(root),
        )
        ...
        if not (check_ghcr.stdout and check_ghcr.stdout.strip()) and not (
            ...
            if not base_image_local:
                ...
                if pull_result.returncode == 0:
                    continue
            print(f"  Building base image for {arch} locally...")
            r = subprocess.run(
                [
                    "docker",
                    "buildx",
                    ...
                    "--tag",
                    base_img_local_tag,
                    *(["--tag", base_img_ghcr] if not base_image_local else []),
                    "--load",
                    "-f",
    ...
    for arch in ["amd64", "arm64", "arm7"]:
        ...
        if base_image_local:
            base_from = f"{base_image_local}:{arch}"
            mod = re.sub(r"(?m)^ARG BASE_IMAGE=.*\n?", "", template_content)
            mod = mod.replace("FROM ${BASE_IMAGE}", f"FROM {base_from}")
        else:
        ...
        arch_df = root / docker_dir / "microservices" / f"Dockerfile.{system}_{module}.{arch}"
