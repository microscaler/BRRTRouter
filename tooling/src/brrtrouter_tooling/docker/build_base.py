...
def run(
    ...
    tags = [tag_local, tag_ghcr]
    if tag_kind_registry:
        tags.append(f"localhost:{KIND_REGISTRY_PORT}/{base_image_name}:latest")
    if tag_dockerhub and dockerhub_org:
        tags.append(f"docker.io/{dockerhub_org}/{base_image_name}:latest")
    if dry_run:
