"""`hauliage build` (host-aware cargo/cross; microservices/ workspace)."""

import sys
from pathlib import Path

from brrtrouter_tooling.workspace.build.host_aware import run as run_host_aware
from brrtrouter_tooling.workspace.build.microservices import (
    build_microservice,
...
def run_build(args, project_root: Path) -> None:
    ...
    if target == "microservice":
        if not arch:
            print(
                "hauliage build microservice: missing service name (e.g. identity)",
                file=sys.stderr,
            )
            sys.exit(2)
