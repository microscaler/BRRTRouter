...
def _get_port_from_registry(project_root: Path, service_name: str) -> int | None:
...
def update_tiltfile(
    ...
    gen_cargo = f"'./microservices/{service_name}/gen/Cargo.toml'"
    impl_cargo = f"'./microservices/{service_name}/impl/Cargo.toml'"
    if m:
        deps = [d.strip().strip("'\"") for d in m.group(2).split(",") if d.strip()]
...
def _ensure_impl_scaffold(project_root: Path, suite: str, service_name: str) -> None:
    ...
    if not gen_dir.exists():
        msg = f"gen/ not found at {gen_dir}; run 'hauliage gen hauliage {service_name}' first"
