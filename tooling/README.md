...
- **Collision policy:** The merge fails when two source operations produce the same public path and HTTP method, or when any two operations in the suite reuse an `operationId`. Service-prefixed public paths may disambiguate URLs, but they never hide ambiguous operation identities.
