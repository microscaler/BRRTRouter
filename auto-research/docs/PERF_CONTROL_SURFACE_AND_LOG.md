...
||| *2026-04-22* | **Fix validator cache race: status-code-aware cache keys** — multiple response schemas with the same HTTP status code (e.g. both returning 200 OK) collided in the cache key, causing duplicate key errors. Fixed by making cache keys include the status code so each response schema gets its own entry. No bench delta (race condition, not perf). Commit `392d820`. |
