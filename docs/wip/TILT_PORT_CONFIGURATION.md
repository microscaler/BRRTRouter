...
Tilt web UI now runs on **port 10353** by default (changed from 10350 to avoid common conflicts).

## 🔧 Changing the Tilt Port

If port 10353 is already in use, you have several options:
...
tilt_port = cfg.get('tilt_port', '10353')  # Change 10353 to your preferred port
```
...
| **Tilt Web UI** | 10353 | HTTP | `TILT_PORT` | Dev dashboard |
...
```bash
# Check if port 10353 is in use
lsof -i :10353

# Check all BRRTRouter ports at once
lsof -i :8080 -i :3000 -i :5432 -i :6379 -i :9090 -i :16686 -i :10353

# Kill process on specific port (if needed)
kill -9 $(lsof -ti:10353)
```
...
- **10353-10399**: Available for Tilt and dev tools
...
```bash
#!/bin/bash
PORTS=(8080 3000 5432 6379 9090 16686 10353)
ALL_FREE=true
...
**Default Tilt Port**: 10353 (configurable via `TILT_PORT`)
