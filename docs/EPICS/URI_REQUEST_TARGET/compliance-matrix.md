...
| REQ-URI-VALIDATE | Rebuilt target parses as URI | RFC 9110 §7 | dual `http` Uri | **Partial** | Dual-stack gated (10.8); not a full RFC 3986 validator |
...
| REQ-OPENAPI-STYLE | style/explode fidelity on rebuild | OpenAPI 3 | `openapi_query` | **Pass** | Story 10.9; `openapi_query_*`; golden P6 roundtrip |
| REQ-ERROR-TAXONOMY | Uri-build ≠ upstream 502 | Ops | `proxy_error_response` | **Pass** | Story 10.7; `proxy_error_taxonomy_*` |
| REQ-HTTP-STACK | Unify http 0.2 / 1.0 URI handling | Internal | `RequestTarget` bridge | **Pass** | Story 10.8 Decision B; dual-stack goldens |
| REQ-FUZZ | Property/fuzz no-panic + Uri-OK | QA | `uri_property_tests` | **Pass** | Story 10.10; fixed seed `0x1010_c0ff_ee` |
