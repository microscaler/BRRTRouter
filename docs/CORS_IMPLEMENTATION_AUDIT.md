...
| HTTP CORS (global Bearer, cookie API key) | `tests/cors_http_security_schemes_tests.rs` | Minimal OpenAPI fixtures in `tests/fixtures/` — **`http_cors_preflight_global_bearer_returns_401_without_authorization`**, **`http_cors_preflight_global_bearer_returns_200_with_valid_bearer`**, **`http_cors_preflight_global_cookie_api_key_returns_401_without_cookie`**, **`http_cors_preflight_global_cookie_api_key_returns_200_with_session_cookie`**. |
