...
2. **`Vary` (framework + app responsibility)** — **helper available**
  CORS sets **`Vary: Origin`**, or **`Vary: Origin, Access-Control-Request-Private-Network`** when **Private Network Access** is enabled. The framework does **not** merge other tokens automatically. **`merge_vary_field_value`** (`brrtrouter::middleware::merge_vary_field_value`) merges an existing comma-separated `Vary` value with extra field names (dedupes case-insensitively; handles `*`). **Apps** still choose when to call it — see `[CORS_OPERATIONS.md](./CORS_OPERATIONS.md#vary-merging)`.
...
| HTTP CORS (global Bearer, cookie API key, OR schemes) | `tests/cors_http_security_schemes_tests.rs` | Minimal OpenAPI fixtures in `tests/fixtures/` — Bearer / cookie / **OR ApiKey∨Bearer** preflights (`http_cors_preflight_security_or_*`). CORS middleware uses **`with_metrics_sink`** in the fixture. |
