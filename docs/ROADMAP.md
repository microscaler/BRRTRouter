...
- **REST status from typed handlers:** See [PRD: Typed handlers & REST HTTP status](PRD_TYPED_HANDLER_HTTP_STATUS.md) (near-term phases + long-term deliverables: multi-status codegen, `components.responses`, 204/HEAD, pool parity, SDK alignment).
...
- ~15ms latency (excluding handler execution) and ~200-400ms under load with business logic
