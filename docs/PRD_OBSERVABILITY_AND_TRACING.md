...
> **Superseded as architectural authority:** [`microscaler-observability/docs/PRD.md`](../microscaler-observability/docs/PRD.md) (v0.4+). This file stays for **historical** BRRTRouter audit notes and v0.3 phase labels; cross-repo sequencing and ownership live in the master PRD.
...
- **N8 — Installing an OTEL Metrics SDK (`MeterProvider`) in BRRTRouter.** Per the master PRD §4 and [`lifeguard/docs/OBSERVABILITY_APP_INTEGRATION.md`](../lifeguard/docs/OBSERVABILITY_APP_INTEGRATION.md), Lifeguard holds a **local** `SdkMeterProvider` for Prometheus scrape text only (no global `MeterProvider` race with the host). BRRTRouter MUST NOT install a global `MeterProvider`. All BRRTRouter metrics stay on the hand-rolled Prometheus-text `/metrics` endpoint, concatenated with Lifeguard's `prometheus_scrape_text()` (Phase O.6). OTLP metrics export, if ever needed downstream, is the OTEL Collector's job via its `prometheus` receiver → `otlp` exporter — out of scope here.
