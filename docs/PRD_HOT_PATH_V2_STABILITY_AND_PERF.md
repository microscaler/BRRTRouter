...
- **Measurement:** 2000u × 600s post-R.1 bench returned **60,611 req/s / p50 28ms / p95 70ms / p99 110ms** vs the 66,484 / 26 / 64 / 98 baseline captured earlier the same day.
    ...
    - **Correct diagnosis (2026-04-18, post-R.2 rerun — see §Phase R rerun below):** the apparent "regression" was **not** thermal at all. It was log-pipeline saturation → `SIGABRT` (exit 134). The BRRTRouter telemetry architecture is `tracing` → stdout (JSON) → Promtail → Loki, plus OTEL spans to the collector — stdout is the Promtail source, not a human console, so "only startup prints to console" is an observability invariant, not a hot-path constraint. The SIGABRT happened because two categories of log emission violated that invariant under load: (i) per-request `warn!`s polluted both stdout and Loki at thousands of events/sec, saturating the local tracing dispatcher and the stdout write buffer long before Promtail could drain them; (ii) a raw `may::println!` bypassed `tracing` entirely and wrote to stdout on every coroutine spawn, contending with the tracing writer for the stdout FD lock. Three concrete sources, diagnosed under the correct architecture:
        - **`router/core.rs`** — `warn!("Slow route matching detected")` fired per-request when radix lookup > 1 ms. At 2000u load with a radix threshold below the scheduler noise floor, this fires on ~every multi-segment dynamic path. Wrong log level for Loki (alerts that fire per-request are noise, not signal) *and* wrong write volume for the stdout→Promtail pipeline. Demote to `debug!` and raise threshold to 10 ms.
        - **`middleware/memory.rs`** — `warn!("High memory growth detected")` fired on every 10 s poll whenever RSS delta > 100 MB. The 10 s cadence is perfectly appropriate for Loki — this is exactly the operational signal ops wants — but the **100 MB threshold** trivially fires on a legitimate 2000u ramp (coroutine stacks alone are ~70 MB at default config, plus connection buffers, plus `may` internal state). Keep as `warn!`, raise threshold to 500 MB so it fires on anomalous growth, not on routine ramp. Sustained/rate-based leak detection is the Phase 6.7 soak harness.
        - **`runtime_config.rs`** — forced an odd stack size unconditionally, triggering `may::println!` in `coroutine_impl` on every coroutine spawn. This output *bypasses `tracing` entirely* — it lands as raw text on stdout, never reaching Promtail/Loki, pure bench debris with no operational value. Under 2000u that's thousands of synchronous writes per second contending for the stdout FD lock. Gate behind `BRRTR_TRACK_STACK_USAGE=1` so stack tuning stays available for deliberate sessions but does not fire by default.
        The "thermal drift" was a red herring — the machine was fine, the server was crashing because two log sources violated the stdout→Promtail pipeline's throughput assumptions and one log source bypassed it entirely.
...
1. `src/router/core.rs` — `warn!("Slow route matching detected")` → `debug!` with threshold raised 1 ms → 10 ms. **Rationale:** per-request events at WARN pollute Loki with duplicate alerts and saturate the stdout→Promtail pipeline at any sink; 10 ms is the threshold at which "slow lookup" is actually an operational signal rather than scheduler noise.
2. `src/middleware/memory.rs` — `warn!("High memory growth detected")` **kept at `warn!`** (it's a legitimate 10 s-cadence ops signal for Loki), threshold raised **100 MB → 500 MB** so it only fires on anomalous growth, not on routine 2000u ramp. Memory-statistics companion line kept at `info!`.
3. `src/runtime_config.rs` — odd-stack-size opt-in gated behind `BRRTR_TRACK_STACK_USAGE=1`. This is a **raw `may::println!`** that bypasses `tracing` entirely (never reaches Promtail/Loki); thousands of direct stdout writes per second under 2000u load are pure bench debris.
