...
def run_goose_test(
    host: str, users: int, run_time: str, hatch_rate: int, output_file: Path
) -> subprocess.CompletedProcess:
    """Run Goose load test and capture output."""
    print("🦆 Running Goose load test...")
    print(f"   Host: {host}")
    print(f"   Users: {users}")
...
def format_comparison_report(comparison: Dict) -> str:
    ...
    if comparison["improvements"]:
        ...
        for imp in comparison["improvements"]:
            ...
            lines.append(
                f"  • {metric_name}: {imp['baseline']:.2f} → {imp['current']:.2f} ({imp['change_pct']:+.1f}%)"
            )
        lines.append("")
    ...
    if comparison["regressions"]:
        ...
        for reg in comparison["regressions"]:
            ...
            lines.append(
                f"  • {metric_name}: {reg['baseline']:.2f} → {reg['current']:.2f} ({reg['change_pct']:+.1f}%)"
            )
        lines.append("")
    ...
    for metric_key, metric_name in key_metrics:
        if metric_key in comparison["changes"]:
            ...
            lines.append(
                f"  {metric_name:25} {change['baseline']:>10.2f} → {change['current']:>10.2f} ({change['change_pct']:>+6.1f}%)"
            )
...
def main():
    ...
    parser.add_argument(
        "--run-time", default="60s", help="Test duration (default: 60s)"
    )
    parser.add_argument(
        "-r",
        "--increase-rate",
    ...
    parser.add_argument("--baseline", help="Baseline label to compare against")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("/tmp/goose_metrics"),
        help="Directory for metrics storage (default: /tmp/goose_metrics)",
    )
    parser.add_argument(
        "--skip-warmup", action="store_true", help="Skip server warmup phase"
    )
    parser.add_argument(
        "--skip-server-check", action="store_true", help="Skip server health check"
    )
    ...
    for run_num in range(1, 4):
        ...
        output_file = Path(
            f"/tmp/goose_{args.label}_{args.users}users_run{run_num}.txt"
        )
        ...
        print(
            f"  Run {run_num}: {run_metrics.get('throughput_req_per_sec', 0):,.0f} req/s, "
    ...
    print(
        f"  Failures: {metrics.get('total_failures', 0)} ({metrics.get('failure_rate', 0):.2f}%)"
    )
