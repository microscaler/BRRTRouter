...
def parse_test_results(output):
    ...
    for line in lines:
        ...
        if test_match:
            ...
            tests.append({"name": test_name, "result": result, "suite": current_suite})
    ...
    for test_name, test_runs in test_results.items():
        ...
        if success_count > 0 and failure_count > 0:
            ...
            flakey_tests[test_name] = {
                ...
                "failed_iterations": [
                    run["iteration"] for run in test_runs if not run["success"]
                ],
            }
...
def main():
    ...
    for i in range(1, 26):
        ...
        print(
            f"  {i:2d}/25: {status} - {duration:6.2f}s - P:{result['passed']} F:{result['failed']} I:{result['ignored']}"
        )
    ...
    for result in results:
        ...
        print(
            f"| {result['iteration']:2d}  | {status:6s} | {duration:8s} | {result['passed']:6d} | {result['failed']:6d} | {result['ignored']:7d} | {failed_tests_str}"
        )
    ...
    if flakey_tests:
        ...
        print()
        for test_name, stats in sorted(
            flakey_tests.items(), key=lambda x: x[1]["success_rate"]
        ):
            print(f"   • {test_name}")
            print(
                f"     Success Rate: {stats['success_rate']:.1f}% ({stats['successes']}/{stats['total_runs']})"
            )
    ...
    if stable_failing:
        ...
        for test_name in sorted(stable_failing.keys()):
            print(
                f"   • {test_name} (failed in all {stable_failing[test_name]['total_runs']} runs)"
            )
    ...
    print("=" * 60)
    print(
        f"Total Unique Tests: {len(stable_passing) + len(stable_failing) + len(flakey_tests)}"
    )
