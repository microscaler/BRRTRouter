...
def format_change(
    baseline: float, current: float, unit: str = "", lower_is_better: bool = True
) -> str:
    ...
    if abs(change_percent) < 1.0:
        ...
        color = ""
    elif (lower_is_better and change_percent < 0) or (
        not lower_is_better and change_percent > 0
    ):
...
def compare_metrics(
    ...
    for metric in [
        "avg_route_match_latency_us",
        "p50_route_match_latency_us",
        "p95_route_match_latency_us",
        "p99_route_match_latency_us",
    ...
    print(
        f"  └─ Total Requests: {format_change(b_total, c_total, '', lower_is_better=False)}"
    )
    ...
    if calculate_change_percent(b_lock_p99, c_lock_p99) > 20:
        regressions["p99_lock_acquisition_us"] = calculate_change_percent(
            b_lock_p99, c_lock_p99
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare performance metrics from two test runs"
    )
