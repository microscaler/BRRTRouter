...
def run_performance_test(
    host: str, users: int, run_time: str, hatch_rate: int, report_file: str
...
def generate_system_info_markdown(
    system_info: Dict[str, str], config: Dict[str, Any]
) -> str:
...
def generate_report_markdown(
    ...
    md.append("|--------|-------|")
    md.append(
        f"| Average Lock Time | {metrics.get('avg_lock_acquisition_us', 0):.2f}µs |"
    )
    ...
    md.append("## Files")
    md.append("")
    md.append("- [Full Metrics (JSON)](metrics.json)")
    md.append("- [Goose HTML Report](goose-report.html)")
    md.append("- [System Information](system-info.md)")
    md.append("")
...
def main():
    ...
    parser.add_argument(
        "--users",
        type=int,
        default=int(os.environ.get("USERS", "100")),
        help="Number of concurrent users (default: 100)",
    )
    parser.add_argument(
        "--run-time",
        default=os.environ.get("RUN_TIME", "5m"),
    ...
    parser.add_argument(
        "--baseline",
        type=Path,
        default=Path("baseline-metrics.json"),
